...
from spectre.src.utils.renderer import SRenderY, set_rasterizer
...
class SPECTRE(nn.Module):
    ...
    def _create_model(self, model_cfg):
        ...
        self.E_expression = PerceptualEncoder(model_cfg.n_exp, model_cfg).to(
            self.device
        )
        ...
        # resume model
        model_path = f"{get_project_path()}/models/spectre_model.tar"
        if os.path.exists(model_path):
            ...
            if "deca" in list(self.checkpoint.keys())[0]:
                for key in self.checkpoint.keys():
                    ...
                    k = key.replace("deca.", "")
                    if "E_flame" in key:
                        processed_checkpoint["E_flame"][k.replace("E_flame.", "")] = (
                            self.checkpoint[key]
                        )  # .replace("E_flame","")
                    elif "E_expression" in key:
                        processed_checkpoint["E_expression"][
                            k.replace("E_expression.", "")
                        ] = self.checkpoint[
                            key
                        ]  # .replace("E_flame","")
                    else:
            ...
            try:
                m, u = self.E_expression.load_state_dict(
                    processed_checkpoint["E_expression"], strict=True
                )
                ...
                print(
                    f"Missing keys {e} in expression encoder weights. If starting training from scratch this is normal."
                )
        else:
            raise ImportError(f"please check model path: {model_path}")
    ...
    def decompose_code(self, code, num_dict):
        ...
        for key in num_dict:
            ...
            if key == "light":
                dims_ = (
                    code_dict[key].ndim - 1
                )  # (to be able to handle batches of videos)
                code_dict[key] = code_dict[key].reshape(
                    *code_dict[key].shape[:dims_], 9, 3
                )
        return code_dict

    def encode(self, images, requires_grad=False):
        if requires_grad:
    ...
    def decode(
        ...
        if rendering:
            if self.cfg.model.use_tex:
                ...
                shape_images, _, grid, alpha_images, pos_mask = (
                    self.render.render_shape(
                        verts,
                        trans_verts,
                        h=h,
                        w=w,
        ...
        if vis_lmk:
            landmarks3d_vis = self.visofp(
                ops["transformed_normals"]
            )  # /self.image_size
        ...
        if return_vis:
            ## render shape
            shape_images, _, grid, alpha_images, pos_mask = self.render.render_shape(
                verts,
                trans_verts,
                h=custom_size,
                w=custom_size,
